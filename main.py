...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Skrypt do parsowania argumentów')
    ...
    parser.add_argument('--json_file', type=str, help='.json do wczytania/zapisu')
    parser.add_argument('--xml_file', type=str, help='.xml do wczytania/zapisu')
...
def verify_xml_syntax(file_path):
    try:
        ET.parse(file_path)
        print(f'Plik {file_path} m poprawną składnię .xml.')
    except FileNotFoundError:
        print(f'Plik {file_path} nie istnieje.')
    except ET.ParseError as e:
        print(f'Błąd składni pliku .xml: {e}')
